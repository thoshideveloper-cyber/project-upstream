"""The company database every firm starts with — real organisations, no Faker.

This is the one surface of the product that ships pre-filled. The sourcing pool is the
firm's *company database*: the inventory an analyst searches before a deal exists, so it
has to be worth searching on day one. Everything else in the app (master lists, contacts,
schedules, projects) starts empty and arrives from the client workbooks an analyst
uploads — the pool is the only standing context, which is exactly why it may not be
invented. Every row here is a real organisation whose name and headquarters were taken
from a public source (cited per block below) in August 2026.
...
guessed domain is worse than no domain. Headcount and revenue are deliberately absent:
those arrive with the client's own researched workbooks, and an estimate on a screen an
analyst makes calls from is worse than a blank.

Each row carries two research dimensions, both taken from the block it was sourced from
rather than guessed per company:
- ``segment`` — ``TARGET`` (sell-side inventory) or ``INVESTOR`` (buy-side /
  capital-raise counterparties): the two sides of the firm's real deal flow.
- ``sector`` — the research bucket the list came from (IT services, managed services,
...
SECTOR_IT = "IT services & consulting"
SECTOR_MSP = "Managed services & security"
SECTOR_PE = "Private equity"
SECTOR_VC = "Venture capital"
...
# ── India PE — verified individually (the client's home market) ──────────────
...
# ── Venture capital — the growth-stage end of the same buy side ──────────────
# Split out of the PE blocks on purpose: a16z, Sequoia and the India funds below run
# venture strategies, and a "Private equity" label on them would be a wrong fact on a
# screen an analyst filters by.
_VC: list[PoolRow] = [
...
# Sector travels with the *block*, never guessed per company — a row's bucket is the
# list it was researched from.
BLOCKS: list[tuple[list[PoolRow], str]] = [
    (_IT_SERVICES, SECTOR_IT),
    (_MSP, SECTOR_MSP),
    (_PE_GLOBAL, SECTOR_PE),
    (_PE_ASIA, SECTOR_PE),
    (_PE_INDIA, SECTOR_PE),
    (_VC, SECTOR_VC),
]

DATASET: list[PoolRow] = [row for block, _ in BLOCKS for row in block]
...
def rows() -> list[dict]:
    ...
    return [
        ...
        for block, sector in BLOCKS
        for name, hq, domain, segment in block
    ]
